from satimol.exceptions import STMLSyntaxError
from satimol.parser import Expr, Node
from satimol.signature import Signature
from satimol.valformat import ValFormatRegistry
from satimol.log import debug
...
class ValTag(EmptyTag):
    ...
    def genCode(self, codeStream):
        ...
        codeStream.writeln('__h.satimol.write_val(%r, OUTPUT, %r)' % (expr, fmt))
...
class CacheTag(EmptyTag):
    ...
    modules=[('satimol.timeutil', '_timeutil')]
    _top=True

    def genCode(self, codeStream):
        when=self._parsed_args['when']
        s="__expiration=__h._timeutil.convertEither(%r)" % when
        codeStream.writeln(s)
